...
#define trainingData "dota2Train_new.csv"
#define testingData "dota2Test_new.csv"
...
double entropy(vector <double> counts)
{
	...
	// Soma o valor de entropia de forma paralela utilizando reduction
	#pragma omp paralle for reduction(+:entropy)
	for(i=0;i<counts.size();i++){
...
// attr: candidate attributes for splitting attribute, attr[i]=1 if already used
// data: data row nos(in the file and index in "fileContent" vector) used for calculating information gains
void decision(vector<int> attr,vector<int> data,node *root, int n)
{
	//if(n >= 5) return;
	//to be deleted
	// printf("Data Points:\n");
	...
	for(i=0,it=dividedData.begin();it!=dividedData.end();it++,i++){
		...
		// Paralelização de função paralela
		#pragma omp task
		decision(attr, it->second, childNode,n + 1);
	}
	// Espera os filhos que estão sendo executados de forma paralela, terminarem suas execuções
	#pragma omp taskwait
...
void test(node* root, int* decisionGT)
{
	...
	neg=0;
	readCSV("testing");
	// Todo o for não possui dependência de dados de modo que atrapalhe a paralelização
	// Cada thread terá suas prórprias variáveis (por isso a não dependência de dados e quase todas as variáveis serem privadas)
	// Pelo fato do falor de pos e neg ser incrementado para todas as threads, nele é utilizado um reduction
	#pragma omp parallel for private(noResult,attr,attrVal,j,flag,temp) reduction(+:pos,neg) schedule(guided,1000)
...
int main()
{
	...
	// Popula os arrays de forma paralela. Cada for é executado por uma única thread sempre que for possível
	// Os elementos compartilhados possuem essa visibilidade por estarem sendo utilizados por múltiplas threads simultanemante, de modo que o valor alterado fique no original
	// A variável i é privada, pois seu valor não deve ser alterado para os dois "for", pois cada for possui seu próprio valor de i
	#pragma omp parallel shared(numOfDataEle,numOfAttrib,data,attr) private(i) num_threads(nThreads)
	{
		#pragma omp single
		{
	...
	//Mesma coisa de cima só que com task
	#pragma omp parallel num_threads(nThreads)
	{
		#pragma omp single
		{
			// Paralelização de uma tarefa recursiva, dentro do método todas as suas chamadas recursivas
			// São feitas utilizando Task
			#pragma omp task
			decision(attr,data,root,0);
		}
	}
	
	// test decision tree
	test(root,decisionGT);

	int count = 0;
	double acertos = 0;
  string output;
	ifstream arq;
	arq.open("dota2Test_GT.csv");
  if(arq.is_open()){
    while(!arq.eof()){
      arq >> output;
      if(decisionGT[count] == stoi(output))acertos++;
      count++;
    }
  }
  cout << "Acertos: " << acertos << "\n";
  cout << "Accuracy: " << acertos/7000 << "\n"; 
	return 0;
}
